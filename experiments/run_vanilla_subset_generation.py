...
def main():
    ...
    for _, rec in df.iterrows():
        ...
        for attack in attacks:
            ...
            out[f'{attack.lower()}_path'] = save_adv(denormalize(adv.numpy()[0]), attack, src_path, tgt_path, rec['attack_type'], args.attacker_model, row_id, args.output_root)
        rows.append(out)

    pd.DataFrame(rows).to_csv(Path(args.output_root) / f'{args.attacker_model}_subset_adv_paths.csv', index=False)
